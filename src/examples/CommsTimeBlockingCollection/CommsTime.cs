...
namespace CommsTimeBlockingCollection
{
	class TickCollector : IProcess
	{
		...
		public void Run()
		{
			var tick_chan = BlockingCollectionChannelManager.GetChannel<bool>(TICK_CHANNEL_NAME).AsRead();
			var tickcount = 0;
			var rounds = 0;
				...
				while (tick_chan.Read())
				{
					...
					if (tickcount >= TICK_COUNT)
					{
						...
						if (++rounds >= MEASURE_COUNT)
							BlockingCollectionChannelManager.GetChannel<bool>("0->1").Retire();
					}
				}
			}
			catch(RetiredException)
			{
				...
				BlockingCollectionChannelManager.GetChannel<bool>(TERM_CHANNEL_NAME).Retire();
			}
		}
	}
	...
	class CommsTime : IProcess
	{
		...
		public void Run()
		{
			...
			var chan_write = BlockingCollectionChannelManager.GetChannel<bool>(write_chan_name).AsWrite();
			var chan_read = BlockingCollectionChannelManager.GetChannel<bool>(read_chan_name).AsRead();
			var tick_chan = BlockingCollectionChannelManager.GetChannel<bool>(TickCollector.TICK_CHANNEL_NAME).AsWrite();
